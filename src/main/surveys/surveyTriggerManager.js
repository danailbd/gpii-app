...
fluid.defaults("gpii.app.surveyTriggerManager", {
    ...
    invokers: {
        ...
        registerTimeout: {
            ...
            args: [
                "{that}.options.triggerTypes.surveyTimeout",
                "{rulesEngine}",
                "{arguments}.0"
            ]
        }
...
gpii.app.surveyTriggerManager.handleRuleSuccess = function (that, triggerType, payload) {
    if (triggerType === that.options.triggerTypes.surveyTrigger) {
        ...
        that.rulesEngine.removeRule(triggerType);
    } else if (triggerType === that.options.triggerTypes.surveyTimeout) {
...
gpii.app.surveyTriggerManager.registerTimeout = function (triggerType, rulesEngine, timeoutData) {
    rulesEngine.addRule(triggerType, timeoutData.conditions, timeoutData);
};
